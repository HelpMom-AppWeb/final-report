...
| Sprint Goal               | El objetivo es completar la conexión entre la interfaz de usuario (frontend) y los servicios del servidor (backend) para todas las funciones principales de la aplicación. Esto asegura que lo que el usuario ve y con lo que interactúa funcione correctamente con la lógica y los datos que maneja el sistema. Simultáneamente, se busca mejorar la experiencia del usuario. Esto implica hacer la aplicación más intuitiva, rápida y agradable de usar, resolviendo posibles puntos de fricción. Finalmente, se debe preparar la aplicación para su lanzamiento definitivo. |
